...
- **点击的前台契约**：`click` / `tap` / `press` / `dclick` / `rclick` 一律先确认落点窗口
  ...
  "莫名其妙不弹出来"的主因。
- **被拒绝时先查 `foreground`，别急着 `--anyway`**：
  ```powershell
  dsh-ui foreground          # 前台窗口: Microsoft Edge — "…"  pid=28356  class=Chrome_WidgetWin_1 …
  dsh-ui foreground --json
  ```
  一行告诉你**谁在前台**。实测最常见的两个元凶：① **某个模态对话框**占着前台（此时应用内
  所有坐标都点不动，先把对话框关掉）；② **你自己的控制台/浏览器** —— 每条 shell 命令都可能把
  焦点交给宿主窗口，刚 `win focus` 过的应用未必还在前台。`--anyway` 只会把这一击送到那个前台
  窗口上，通常仍然点不到目标。`win focus` 失败时，警告里也会报出当前前台窗口。
- 模态框**位置会漂移**（实测 LabVIEW 的「认证」框两次调用之间移动了 14px）：每次都重新用
  OCR 定位按钮（`find-text "取消" --list`），不要复用上一次的坐标。
